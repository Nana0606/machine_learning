...
def train(mnist):
    ...
    with tf.name_scope("biases1"):
        ...
        tf.summary.histogram('weights1', biases1)
    with tf.name_scope("weights2"):
        weights2 = tf.Variable(tf.truncated_normal([LAYER1_NODE, OUTPUT_NODE], stddev=0.1))
        tf.summary.histogram('weights1', weights2)
    with tf.name_scope("biases2"):
        biases2 = tf.Variable(tf.constant(0.1, shape=[OUTPUT_NODE]))
        tf.summary.histogram('weights1', biases2)
    ...
    with tf.name_scope("loss"):
        ...
        tf.summary.scalar('loss', loss)  # loss是一个数字
    ...
    with tf.Session() as sess:
        merged = tf.summary.merge_all()    # 将所有需要绘图的参数整合
        ...
        for i in range(TRAINING_STEPS):
            if i % 1000 == 0:
                ...
                print('After %s training steps, accuracy in validation data is %g, accuracy in test data is %g' % (i, validate_acc, test_acc))
                result = sess.run(merged, feed_dict=validate_feed_dict)  # merged也需要被run
                writer.add_summary(result, i)  # i是记录的步数
