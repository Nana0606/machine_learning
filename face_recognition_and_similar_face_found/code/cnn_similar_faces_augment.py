...
"""
训练集中使用增强数据的数据进行训练，使用CNN进行相似度表情发现
"""
import os
import numpy as np
from keras.models import Sequential
from keras.layers import Dense, Activation, Convolution2D, MaxPooling2D, Flatten, Dropout,BatchNormalization,initializers
from keras.optimizers import Adam
from sklearn.model_selection import train_test_split
import tensorflow as tf
from keras.utils import np_utils
import keras
from final_practice.data_augmentation import data_augment
from final_practice.utils import read_image
...
def cnn_model():
    ...
    model.add(Convolution2D(
        ...
        filters=32,
        kernel_size=3,
        strides=1,     # Convolution2D中的strides设置为1
        padding="same",
...
if __name__ == '__main__':
    ...
    X_train, y_train = read_image()     # 增强训练集数据
    model = cnn_model()
    ...
    print(X_test.shape)
    print(y_test.shape)
    X_train = np.reshape(X_train, (-1, X_train.shape[1], X_train.shape[2], 1))   # 需要4维数据，这里将3维转化为4维
